"""
Error Handler for SurveyMonkey Migration
...
class ErrorHandler:
    ...
    def _create_fallback_transformation(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if item_type == 'user':
        ...
        elif item_type == 'survey':
            return {
                'name': item.get('title', item.get('name', 'Migrated Survey')),
                'description': 'Survey migrated with errors - manual review required',
                'steps': [{
                    'name': 'Review Migration',
                    'type': 'task',
                    'description': 'This survey had errors during migration'
                }],
                'metadata': {
                    'migration_error': True,
                    'original_id': item.get('id')
            ...
            return {
                'name': item.get('name', item.get('title', 'Migrated Item')),
                'type': item_type,
                'metadata': {
                    'migration_error': True,
                    'original_id': item.get('id')
